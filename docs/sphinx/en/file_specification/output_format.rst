...
-  In fermion mode, two-site observables may cover the 4 x 4 window and may be built from ``ops = [A, B]``. The value is the expectation value of the operator on the source site and the target site. With ``meanfield_env = true``, longer-distance values are evaluated by closing the outside of the window with the simple-update :math:`\lambda` weights.
...
-  In fermion mode, this is :math:`\langle A_s B_t\rangle`, where :math:`A` is on the left site for horizontal correlations and on the lower site for vertical correlations. Pairs of one-site operators with different fermion parity are output as zero. Fermionic correlation functions are also available with ``meanfield_env = true``; this uses the simple-update :math:`\lambda` weights to close the directions transverse to the correlation chain.
