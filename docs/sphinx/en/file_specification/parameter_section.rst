...
- ``fermion``
  ...
  - **Operator and gate matrix elements**: the elements of a two-site operator (``elements`` with indices ``in1 in2 out1 out2``) are the matrix elements :math:`\langle o_1 o_2 | O | i_1 i_2 \rangle` in the ordered two-site Fock basis

    .. math::
       |i_1 i_2\rangle = A^\dagger_1(i_1)\, A^\dagger_2(i_2)\, |0\rangle,

    where :math:`A^\dagger(i)` is the creation-operator string of the local basis state :math:`i` in the fixed internal order, and site 1 is ``source_site``. These matrix elements must be evaluated with the fermionic anticommutation relations. For a spinless site (``physical_dim = 2``) no extra signs appear in nearest-neighbor hopping, but for sites with several fermionic modes they do; e.g. for spinful electrons, :math:`\langle \uparrow\downarrow, 0 |\, c^\dagger_{1\uparrow} c_{2\uparrow} \,| \downarrow, \uparrow \rangle = -1` because :math:`c_{2\uparrow}` anticommutes past :math:`c^\dagger_{1\downarrow}`. In practice, build the two-site Fock space of the involved modes numerically (Jordan-Wigner) and read the matrix elements off, rather than writing signs by hand
  ...
  - In the current version fermion mode supports the ground-state calculation with the simple update (CTM or mean-field environment) and the full update (CTM environment only). ``Use_RSVD``, ``Simple_Gauge_Fix``, finite temperature, time evolution, multi-site observables, ``ops``-form two-site observables, two-site observables at distances larger than 1, skewed unit cells, one-wide cells (``LX < 2`` or ``LY < 2``), correlation functions, and correlation length are not available and are rejected (or forcibly disabled, for the correlation length) when the input is read
  - Notes on the full update:

    - ``fastfullupdate = true`` (the default) is available in fermion mode. The full update supports only the CTM environment, and combining it with ``meanfield_env = true`` is an error
    - The plain path (``fastfullupdate = false``) reconverges the CTM after every bond and is slower than the fast full update (about 9 times for free fermions at ``D = 3``, ``chi = 12``). In fermion mode this reconvergence starts from the environment of the previous bond (a warm start); the bosonic plain path still converges from uniform vectors every time
    - If a ``fastfullupdate = false`` run stops at the forbidden-parity guard above, increase the number of simple-update steps or use the fast full update. Reconverging the CTM from scratch after every bond can settle on a different fixed point when the simple update has not converged sufficiently, and that is the main cause of such a stop
    ...
    - For small bond dimensions (e.g. ``D = 2`` for free fermions) the full update started from a converged simple-update state can *raise* the energy. This is a property of the projected imaginary-time evolution, whose fixed point can lie above the simple-update fixed point when the variational space is too small, not an implementation error. ``D >= 3`` is recommended for fermionic models
  - ``tensor_save`` / ``tensor_load`` are available in fermion mode. The parity ledger of the virtual bonds is written to ``fermion.dat`` in the save directory and validated on load (the physical parity, ``virtual_dim``, ``L_sub``, ``skew``, and the parity of the tensors themselves). Restarting with a larger ``virtual_dim`` is supported, but restarting with a smaller ``virtual_dim`` is not. The expanded components are padded with zeros, so simple update has to be run after loading to use the enlarged space; measurement alone will not populate it
