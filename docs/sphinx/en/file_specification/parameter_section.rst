...
- ``fermion``
  ...
  - Notes on the full update:
    ...
    - The two-site environment of the full update has to conserve fermion parity. If the violation (typically caused by an unconverged CTM) exceeds the threshold, which is the larger of ``1e-8`` and ``convergence_epsilon`` of ``[parameter.ctm]``, the run stops with an error; increase ``iteration_max`` or decrease ``convergence_epsilon`` in ``[parameter.ctm]`` in that case
