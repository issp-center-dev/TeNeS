...
- ``fermion``
  ...
  - In the current version fermion mode supports the ground-state calculation with the simple update (CTM or mean-field environment) and the full update (CTM environment only). With the CTM environment, two-site observables may use explicit ``elements`` or ``ops = [A, B]`` and may be measured in the 4 x 4 window (``|dx| <= 3`` and ``|dy| <= 3``); ``[correlation]`` is also available and writes :math:`\langle A_s B_t\rangle`, with :math:`A` on the left or lower site. Operator pairs with different fermion parity are output as zero. ``Use_RSVD``, ``Simple_Gauge_Fix``, finite temperature, time evolution, multi-site observables, unit cells in which a site is its own nearest neighbor (``LX = 1``, or ``LY = 1`` with ``skew = 0``), and longer-distance Hamiltonian bonds are not available and are rejected when the input is read. Longer-distance two-site observables and correlation functions are not yet available with ``meanfield_env = true``. The correlation length is likewise unavailable: ``tenes_simple`` rejects a ``[correlation_length]`` table for a fermionic model, and the solver disables the measurement with a warning at the point of measurement
...
.. csv-table::
   ...
   ``meanfield_env``,            "Use mean field environment obtained through simple update instead of CTM. Also available in fermion mode, where two-site observables are evaluated by a single-layer contraction (much cheaper than the CTM path, with simple-update-level accuracy)", Boolean, false
