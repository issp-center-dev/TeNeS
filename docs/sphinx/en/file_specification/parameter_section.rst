...
- ``fermion``
  ...
  - Notes on the full update:
    ...
    - The plain path (``fastfullupdate = false``) reconverges the CTM after every bond and is slower than the fast full update. Also, when the full update with ``D >= 3`` is started from a state where the simple update has not converged sufficiently, the CTM can get stuck at a different fixed point and the run can stop at the forbidden-parity guard. In that case, increase the number of simple-update steps or use the fast full update
