...
- ``fermion``
  ...
  - ``tensor_save`` / ``tensor_load`` are available in fermion mode. The parity ledger of the virtual bonds is written to ``fermion.dat`` in the save directory and validated on load (the physical parity, ``virtual_dim``, ``L_sub``, ``skew``, and the parity of the tensors themselves). Restarting with a different ``virtual_dim`` is not supported, because resizing a leg does not preserve its even/odd blocks
