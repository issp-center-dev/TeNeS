...
- ``fermion``
  ...
  - Notes on the full update:
    ...
    - ``fastfullupdate = true`` (the default) is not supported in fermion mode; a warning is printed and the plain full update (the CTM is reconverged after every bond) is used instead. Combining it with ``meanfield_env = true`` is an error
