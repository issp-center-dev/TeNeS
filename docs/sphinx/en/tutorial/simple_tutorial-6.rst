...
One is a solid-like order which exists at a 1/3 filling, where one of three sites is filled in a \ :math:`\sqrt{3}\times\sqrt{3}`\  ordering with wave vector \ :math:`\bm{Q}=(4\pi/3,0)`\  (see the inset of :numref:`fig_tutorial6_hardcore_boson` ).
This long-range order is characterized by the structure factor \ :math:`S(\bm{Q})`\.
...
Using this file, the structure factor \ :math:`S(\bm{Q})`\  can be calculated.
...
The calculation will finish within several minites or several tens of minites.
...
Then, we obtain a graph for the structure factor \ :math:`S(\bm{Q})`\  and the superfluid order parameter \ :math:`|\langle b \rangle|`\  as shown in :numref:`fig_tutorial6_hardcore_boson` (a).
