...
収束したかどうかの判定には、TeNeS では次の2つの条件を用いており、両方が満たされたときに反復を終了します。1つ目は、4つの角転送行列それぞれについて、規格化した特異値スペクトルの反復間の変化が ``convergence_epsilon`` を下回ることです。2つ目は、全サイトの1サイト縮約密度行列について、トレースで規格化したものの反復間の変化（成分ごとの差の最大値）が同じく ``convergence_epsilon`` を下回ることです。
