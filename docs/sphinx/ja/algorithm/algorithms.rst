...
局所磁化のダイアグラムから演算子 :math:`S^z_i` を取り除き、2本の物理脚を縮約せずに残すと、そのサイトの1サイト縮約密度行列 :math:`\rho_i` が得られます。任意の1サイト演算子の期待値は :math:`\mathrm{Tr} (\rho_i O) / \mathrm{Tr} \rho_i` として :math:`\rho_i` から得られるため、TeNeS では :math:`\rho_i` 自体も全サイトについて出力しています（ ``onesite_density_matrix.dat`` ）。規格化していないので、そのトレースは波動関数のノルム :math:`\langle \Psi | \Psi \rangle` そのものになっています。この :math:`\rho_i` は、上で述べた収束判定にも用いられます。フェルミオン模式で CTM 環境を用いる場合、環境は窓ごとに異なる全体位相を持つため、収束判定では :math:`\rho_i` をそのトレースの位相で割ってから比較し、出力される :math:`\rho_i` も同じように正規化されます(トレースは :math:`|\langle \Psi | \Psi \rangle|` になります)。
