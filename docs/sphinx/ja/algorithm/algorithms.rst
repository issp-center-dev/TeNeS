...
収束したかどうかの判定には、TeNeS では次の2つの条件を用いており、両方が満たされたときに反復を終了します。1つ目は、4つの角転送行列それぞれについて、規格化した特異値スペクトルの反復間の変化が ``convergence_epsilon`` を下回ることです。2つ目は、全サイトの1サイト縮約密度行列について、反復間の変化（成分ごとの差の最大値をそのトレースで割ったもの）が同じく ``convergence_epsilon`` を下回ることです。トレースで割る前の差を用いているため、この量には密度行列の「形」の変化とノルム（トレース）の相対変化の両方が含まれ、両者がともに収束することを要求します。

2つ目の条件は、角転送行列の特異値だけでは収束を判定できない場合があるために用いられます。仮想ボンドの次元が1の脚を含み、テンソルネットワークが独立な部分に分解するような格子（例えば、横方向の仮想ボンド次元を1とした擬一次元的な構成）では、角転送行列は実質的にランク1となり、その特異値スペクトルは反復のごく初期から変化しなくなります。このとき、エッジテンソルがまだ収束していなくても収束したと判定され、期待値に無視できない誤差が残ります。1サイト縮約密度行列は角転送行列とエッジテンソルの両方から計算されるため、この状況を検出することができます。この2つ目の条件は ``use_onesite_rdm_convergence`` で無効にできます（既定では有効）。

ただし、上の擬一次元的な構成は収束判定が破綻しうる状況の例として挙げたものであり、**TeNeS がそのような計算を想定・推奨しているわけではありません**。TeNeS が対象としているのは2次元格子です。幅が1になると、角転送行列とエッジテンソルから構成される転送行列がランク1に潰れてしまうため、例えば相関長のように環境テンソルに依存する量は意味を持ちません。一次元系の計算には、その目的に合った他の手法を用いてください。

なお、収束時に期待値に残る誤差は ``convergence_epsilon`` 程度の大きさになります。より高い精度が必要な場合は ``convergence_epsilon`` を小さくしてください。
...
フェルミオン系で CTM 環境を用いる長距離2サイト測定では、2サイト演算子を graded SVD のチャネルに分解します。各チャネルは1次元の補助脚 :math:`\kappa` を持ち、この脚を2サイト間の仮想ボンドに沿ってリレーします。実装では、窓の中でこの脚を受け渡し、各辺で交差符号と仮想ボンドの向きに由来する符号を掛けたうえで、bra 層と ket 層を通常の CTM 縮約に折りたたみます。相関関数では同じチャネルの考え方を用いつつ、 :math:`A_s B_t` を左右の1サイト因子に分解し、 :math:`\kappa` 脚をリレーした転送鎖として伸ばします。

局所磁化のダイアグラムから演算子 :math:`S^z_i` を取り除き、2本の物理脚を縮約せずに残すと、そのサイトの1サイト縮約密度行列 :math:`\rho_i` が得られます。任意の1サイト演算子の期待値は :math:`\mathrm{Tr} (\rho_i O) / \mathrm{Tr} \rho_i` として :math:`\rho_i` から得られるため、TeNeS では :math:`\rho_i` 自体も全サイトについて出力しています（ ``onesite_density_matrix.dat`` ）。規格化していないので、そのトレースは波動関数のノルム :math:`\langle \Psi | \Psi \rangle` そのものになっています。この :math:`\rho_i` は、上で述べた収束判定にも用いられます。フェルミオン系で CTM 環境を用いる場合、環境は窓ごとに異なる全体位相を持つため、収束判定では :math:`\rho_i` をそのトレースの位相で割ってから比較し、出力される :math:`\rho_i` も同じように正規化されます(トレースは :math:`|\langle \Psi | \Psi \rangle|` になります)。
...
``meanfield_env = true`` で物理量を測定する場合、TeNeS は測定するボンド、窓、または相関鎖の外側をこれらの :math:`\lambda` で閉じます。そのため、結果は CTM 縮約ではなく平均場近似になります。
