...
フェルミオン系で CTM 環境を用いる場合、環境は任意の全体位相(実テンソルでは符号)を持つため、:math:`\rho_i` はそのトレースの位相で割ってから出力されます。したがってトレースは :math:`|\langle \Psi | \Psi \rangle|` となり、``onesite_obs.dat`` の ``op_group`` = -1 の値(そのまま出力されます)とはその位相の分だけ異なります。
