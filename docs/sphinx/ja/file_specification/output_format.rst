...
全サイトの規格化された1サイト縮約密度行列が出力されます。
行番号は bra (conj 側)、列番号は ket を表し、各サイトで trace は1に規格化されています。
各行5列からなります。
...
規格化された1サイト縮約密度行列が、 ``onesite_density_matrix.dat`` と同じ行・列規約で出力されます。
...
例えば、規格化された1サイト縮約密度行列は ``FT_onesite_density_matrix.dat`` に出力され、1列目に逆温度、2列目以降に site、row (bra)、col (ket)、実部、虚部が並びます。
