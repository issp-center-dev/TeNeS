...
``use_onesite_rdm_convergence`` が ``true`` の場合、角転送行列の特異値スペクトルに加えて、trace 規格化後の1サイト縮約密度行列の反復間距離を用いて CTM の収束を判定します。
距離は全サイト・全行列要素に対する max 要素ノルムです。
次元1の仮想ボンドを含むネットワーク、例えば準1次元系や蜂の巣格子の自明ボンドを含む場合、角スペクトルだけでは収束を誤検知することがあるため、その対策として使われます。
