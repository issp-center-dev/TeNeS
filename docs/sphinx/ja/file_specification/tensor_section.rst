...
``-Lx < skew < Lx`` の範囲で指定します。 ``skew`` と ``skew`` ± ``Lx`` は同じ格子を表すので、範囲外の値はエラーになります。
...
1サイトに複数のフェルミオンモードがある場合(例: スピンあり電子、 ``physical_dim = 4``)は、
**サイト内の生成演算子の順序を1つ固定し、 以後すべてで同じ順序を使ってください** 。
各基底状態は、固定した内部順序で生成演算子を真空に作用させたものとして定義します。
たとえば内部順序を :math:`(\uparrow, \downarrow)` とすると
...
正しく計算するためには、すべての演算子・ゲート・初期状態を同一の規約で書く必要があります。
