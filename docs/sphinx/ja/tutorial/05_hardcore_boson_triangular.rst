...
最後に三角格子上で定義されたハードコアボソン模型の絶対零度のおける相図の計算を紹介します。
...
ハードコアボソン模型では、各サイトのボソン数は0か1に制限されます。
