...
ボソン系と同一(Barthel–Pineda–Eisert, PRA 80, 042333 (2009))。
...
- フェルミオン化するルーチンの呼び出し境界で `ftensor{テンソル, パリティ}` を構築して渡し、
  ...
  自由関数のオーバーロード解決で符号付きに切り替える(**カーネルソースはボソン/フェルミオン
...
解消しなかった(3方式目の規約合わせ失敗)。根本問題は「ボソン用に書かれた手書き CTM の
...
1. **λ の二重計上**: reduced tensor 構築前に Tn へ full-λ dressing をしていた。
   ...
   **状態 = bare Tn の直接縮約**(ボソン CTM 経路と同じ)。full-λ dressing は
...
(a) ボソン対照は同一パラメータで一様収束 → フェルミオン特有、(b) 1D 極限
