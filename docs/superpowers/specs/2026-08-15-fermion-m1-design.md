...
M1 対象ファイルの grep 集計(2026-08-15、`fermion` ブランチ)に基づく。`ftensor` は
この表の全項目を実装しなければならない。表にない mptensor API は M1 では不要。
...
境界ラップのみで、上記以外の API は使わないことを実装時に確認する(使っていれば表を更新)。
