...
**2026-09-05 追記 2(係数 100)**: `Full_update_bond_fermion` が渡す閾値を
`std::max(1.0e-8, 100.0 * peps_parameters.CTM_Convergence_Epsilon)` に改める。
理由: 上の「ε の約 1e-4 倍」は **chi による打ち切りが無い** D=2 chi=8 での実測であり、
一般には成り立たない。D=4 chi=32(CTM は警告なしで収束)では forbidden ratio が
1.5e-8 ≈ 1.5·ε に達し、係数 1 の閾値では収束した CTM を誤って弾いた。
打ち切りのある環境では forbidden 成分は打ち切り誤差の水準まで上がるため、閾値は ε と
同オーダーではなく余裕を持たせる必要がある。ε=1e-8 なら閾値 1e-6 で、CTM 未収束時に
実測された 3.8e-4 は依然弾ける。既定の ε=1e-6 では閾値 1e-4 となり ratio 6e-5 級の
未収束は通過し得るが、その状況では CTM 自身が非収束警告を出すため、この検査は
「静かに壊れた環境」を捕らえる二次的な網と位置づける。
`build_full_update_environment` の既定引数(1e-8)とテスト T2-ii は変更しない。
実測(係数 100 で再実行、D=4 chi=32、ε=1e-8、閾値 1e-6): 8 ボンドの forbidden ratio は
1.20e-10 〜 **1.01e-07**(最大 10·ε)で全通過、CTM 非収束警告なし、1 sweep 完走。
...
6. **独立検証**: 正規化前の窓ノルム(`phase` × 正規化後の `Σ_x N.t(x)·I_wrap.t(x)`)が、閉じた測定経路
   ...
   **さらに(レビュアーの変異実験、2026-09-05)**: wrap マスクを落とした恒等 I_plain(δδ をそのまま)で
   ...
   ことで作れる。このケースで wrap マスクの欠落を捕まえるのは **(3) 窓ノルムの実正性と (4) N_plain の
   半正定値性**である((6) は変異が `phase` と N を同時に −1 倍するので積が不変になり通ってしまう —
   テスト作成者の実測)。比 Σ N·I_plain / Σ N·I_wrap は N の正規化に依らないので、前提アサートは
   変異の有無に関わらず成立し、検査側で落ちる。
