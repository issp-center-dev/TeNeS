...
| 壊した箇所 | 検出するテスト |
|---|---|
| N の入力脚マスク、開放 join、crossing mask、N の transpose 符号 | **T2(iv) の独立 oracle**。E2E では **T5 のみ**(T4 は全偶なのでマスクが +1 に退化し検出しない) |
| Q′ 詰め替え、QR 軸 | T2(iv)、T3(i) |
| Θ の graded 合成、`mask_{m s1}`、初期推定の転置、Tn_new 転置 | **T3(i)** |
| ゲージ因子のパリティ漏れ | T3(iii)、T3(vii) |
| N_plain の符号を**実 CTM 環境・一般ブロック**で | **T2(vi)**(2026-09-04 セッション 2 追加) |

**2026-09-04 セッション 2 の追記(検証行列の穴)**: 上の T2(i)(iv) はいずれも実際の
`Calc_CTM_Environment_density` が作る環境を通さない(自己整合と有限パッチ oracle)。本番で
「FU がエネルギーを上げる」原因探しに 1 セッション費やしたのは、実 CTM 環境で N_plain を
一般ブロックについて検証する手段がテストに無かったからで、契約書 T2-vi / T2-vii / T3-viii として追加した
(内容は契約書 §3.2, §3.4)。また、その原因は実装バグではなく **D=2 の表現力不足による射影虚時間発展の
性質**であり(D=3 では下がる)、T5 は D=3 に改訂予定(契約書 §3.5 の注記)。
記録: `work/fermion/full-update-design/FINDINGS-task5-energy.md`。
...
| T4 | driver、全偶同値(**構造のみ。符号は検出しない**) | AFH 2×2 を fermion=false と fermion=true + parity=[0,0] で走らせ、エネルギー・onesite・twosite を比較。入力は `fastfullupdate = false` を **`[parameter.full_update]`** に置き、`validate_fermion_constraints` が拒否するもの(`[correlation]`、multisite、`ops` 形式の二サイト、`Simple_Gauge_Fix`、`Use_RSVD`)を除き、**`noise = 0.0`** にする(`tensors.cpp:80-90,155-159` により noise≠0 では 2 つの run の初期テンソルが一致しない) | 1e-6 相対(fold CTM と素 CTM の収束差を許容) | `test/fermion/boson_equivalence_full.py.in` |
