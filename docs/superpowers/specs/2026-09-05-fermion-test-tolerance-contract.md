...
- 上の変更で緩むのは、同じケース内で相対的に小さい方の閉じ値だけである。大きい方の
  判定は現状と厳密に同じ値になること。**これをテストで示す**: `scale` を導入する前後で、
  大きい方の `tol` が変わらないことをケース単位で確認した旨を報告に書く。

  **2026-09-05 訂正**: 初版はここに「大きい方(T15 なら C-1)」と書いていたが誤り。
  テスト作成者の実測では T15 の 17 行のうち 8 行で C-2(identity 側)の方が大きい。
  どちらが大きいかはケース依存なので、`scale` は参照側の閉じ値の max を取る
  (実装はそうなっており、17 行で「大きい方は不変・小さい方だけが大きい方の水準まで上がる」ことを確認済み)。
...
**2026-09-05 追記(`[parameter.ctm]` も対象)**: 初版は `[parameter.full_update]` だけを名指ししていたが、
これは見落としである。テスト作成者の切り分けにより、ALS を絞りきっても残差 3.87e-9(余裕 258.6)が
残り、その出所は **CTM** であることが分かった(`[parameter.ctm] convergence_epsilon` の既定値 1e-6 が
TOL と同大という、まったく同型の配置)。1e-12 に絞ると 2.81e-10(余裕 3563)になる。
したがって `[parameter.ctm] convergence_epsilon` も入力に明記して既定値依存を断つこと。
ただし **`iteration_max` の範囲で実際に収束していること**(両 run のログに CTM の非収束警告が
出ていないこと)を確認すること。警告が出たまま数値だけ良くなるのは不可。
この E2E の実行時間を「変更前 / ALS のみ / CTM も」の 3 点で計測して報告に載せ、
ctest 全体(現在 88 秒)に対してこの 1 ケースが不相応に伸びる場合は、警告なしで収束しかつ
実行時間が現状の 2 倍以内に収まる値を選び、選んだ値と却下した値の両方を数値付きで報告すること。
...
- 変異検査: フェルミオン経路の符号を 1 箇所反転させたコピーで T3-iv が**赤になる**ことを
  確認すること。緩めた許容誤差でも符号の誤りは O(1) のずれを生むので検出できるはずである。
  これが確認できなければ緩めすぎである。

  **2026-09-05 訂正**: 初版はこの変異の例として「`wrap_twosite_gate` の swap mask を落とす」を
  挙げていたが、T3-iv では原理的に無効である。T3-iv は全 ledger が偶の対照実験で、テスト自身が
  「wrap は no-op」を前提として主張しているため、grading 符号の変異は定義上ゼロ効果になる
  (実測でも T3-iv は緑のまま。ただし他の 23 test case が赤になるので変異自体は無音ではない)。
  有効な変異はフェルミオン経路の balancing SVD で 1 チャネルの符号を反転させるもので、
  これで T3-iv は `dev = 0.230956`(許容 1e-6 の 2.3e5 倍)で赤になることを確認済み。
