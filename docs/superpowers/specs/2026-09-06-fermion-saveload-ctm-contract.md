...
- **反空洞化**: run1 がフルアップデートを実際に 1 回以上実行したこと(= `update_CTM()` を
  ...
  **2026-09-06 訂正**: 初版はここに「保存された `El_0.dat` の `ndim` を読むなどして」と書いていたが、
  **修正後は使えない**。R1 が入ると `ndim` は SU のみの run でも 4 になるので「full update を通った」
  ことの証拠にならない。テスト作成者が採った `time.dat` の `time full update > 0` が正しい。
  根拠は `iTPS::full_update()`(`src/iTPS/full_update.cpp:196-228`)が
  `num_full_step[group] > 0` の枝でのみ `update_CTM()` を呼び、その後にタイマーを開始する構造で、
  `time_full_update` に加算される経路が他に無いこと。
  併せて run1/run2 の入力の行差分がちょうど 4 行(output・save/load・2 つの `num_step`)であることも
  検査されており、chi や収束条件が 2 run で食い違ったまま比較する事故も塞がれている。
