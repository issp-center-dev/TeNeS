...
**訂正(2026-09-08)**: 初版はここに「`load_tensors()` は内部で `initialize_tensors()` を
呼ばないので別途落とす必要がある」と書いたが、**逆だった**。`load_tensors()` は
`initialize_tensors()`(`tensors.cpp:177`)と `initialize_tensors_density()`(`:253`)からのみ
呼ばれ、どちらもその直後に無効化する。テスト作成者が D6 で「外から順序を作れなかった」と
報告したのは、**その順序が存在しないから**である。`load_tensors()` 側の無効化は現在の
呼び出しグラフでは冗長(将来の呼び出し元に対する防御としてのみ意味がある)。
