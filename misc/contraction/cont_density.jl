...
mutable struct Tensors
    ...
    function Tensors(nrow::Integer, ncol::Integer; orig::Integer=0, pass_as_vector::Bool=false)
        ...
        tns = new(corners, edges, centers, ops, nrow, ncol)
        contract!(tns)
        check_bonds(tns)
        return tns
    end
...
function gen_edges(nrow, ncol; orig=0, pass_as_vector::Bool=false)
    ...
    for icol in 1:ncol
        ...
        push!(edges, Tensor(name, ["$(name)_$dir" for dir in ("l", "r", "b")]))
    end
    for irow in 1:nrow
        i = orig+irow-1
        name = pass_as_vector ? "*(eTr[$(i)])" : "eTr_$(i)"
        push!(edges, Tensor(name, ["$(name)_$dir" for dir in ("t", "b", "l")]))
    end
    for icol in reverse(1:ncol)
        i = orig+icol-1
        name = pass_as_vector ? "*(eTb[$(i)])" : "eTb_$(i)"
        push!(edges, Tensor(name, ["$(name)_$dir" for dir in ("r", "l", "t")]))
    end
    for irow in reverse(1:nrow)
        i = orig+irow-1
        name = pass_as_vector ? "*(eTl[$(i)])" : "eTl_$(i)"
        push!(edges, Tensor(name, ["$(name)_$dir" for dir in ("b", "t", "r")]))
    end
    return edges
end

function gen_centers(nrow, ncol; orig::Integer=0, pass_as_vector::Bool=false)
    centers = Tensor[]
    for irow in 1:nrow
        irow = orig+irow-1
        for icol in 1:ncol
            ...
            bonds =  ["$(bond_base)_$dir" for dir in ("l", "t", "r", "b")]
            push!(bonds, "p_$(irow)_$(icol)")
            push!(bonds, "Cp_$(irow)_$(icol)")
...
function check_bonds(tensors::Tensors)
    ...
    for T in Iterators.flatten((tensors.corners, tensors.edges, tensors.centers, tensors.ops))
        for b in T.bonds
            c = get(counts, b, 0)
            if c>1
                push!(fails, b)
...
function dump_fn_declare(io::IO, tensors::Tensors; lang::AbstractString="cpp", pass_as_vector::Bool=false)
    ...
    if lang=="cpp"
        ...
        println(io, "Contract_density_$(tensors.nrow)x$(tensors.ncol)(")
        if pass_as_vector
        ...
        println(io, "def Contract_density_$(tensors.nrow)x$(tensors.ncol)(")
        if pass_as_vector
