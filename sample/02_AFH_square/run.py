...
nprocs = 1 if len(argv)==1 else int(argv[1])
...
for D in Ds:
    ...
    for num_step in num_step_table:
        ...
        dict_toml["parameter"]["general"]["tensor_save"] = "tensor_save".format(D,num_step)
        ...
        if ns > 0:
            ...
            dict_toml["parameter"]["general"]["tensor_load"] = "tensor_save".format(D,num_pre)
        with open("simple_{}_{}.toml".format(D,num_step), 'w') as f:
            toml.dump(dict_toml, f)
        cmd = "tenes_simple simple_{}_{}.toml -o std_{}_{}.toml".format(D,num_step,D,num_step)
        subprocess.call(cmd.split())
        cmd = "tenes_std std_{}_{}.toml -o input_{}_{}.toml".format(D,num_step,D,num_step)
        subprocess.call(cmd.split())
        if nprocs == 1:
            cmd = "tenes input_{}_{}.toml".format(D,num_step)
        else:
            cmd = "mpiexec -np {} tenes input_{}_{}.toml".format(nprocs,D,num_step)
        subprocess.call(cmd.split())
