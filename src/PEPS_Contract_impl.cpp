...
typename tensor::value_type Contract_1x1(
...
template <class tensor>
typename tensor::value_type Contract_3x1(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_3x2(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_3x3(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_1x4(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_4x1(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_2x4(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_4x2(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_3x4(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
template <class tensor>
typename tensor::value_type Contract_4x3(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
                                                                           ...
                                                                           tensordot(*(eTr[2]),
                                                                                     tensordot(*(Tn[2]
                                                                                               ...
                                                                                               tensordot(tensordot(conj(
                                                                                                         ...
                                                                                                         tensordot(*(Tn[2]
                                                                                                                       [1]),
                                                                                                                   tensordot(tensordot(conj(*(Tn[2][1])), *(op[2][1]), Axes(4), Axes(1)),
                                                                                                                             tensordot(
                                                                                                                                 *(Tn[2]
                                                                                                                                     [0]),
                                                                                                                                 tensordot(
                                                                                                                                         ...
                                                                                                                                         tensordot(
                                                                                                                                                     ...
                                                                                                                                                     tensordot(
                                                                                                                                                         ...
                                                                                                                                                         *(op[3]
                                                                                                                                                         ...
                                                                                                                                                         Axes(
                                                                                                                                                             1)),
                                                                                                                                                     tensordot(
                                                                                                                                                         *(eTb[0]), tensordot(*(C[3]), *(eTl[3]), Axes(1), Axes(0)),
                                                                                                                                                         Axes(
                                                                                                                                                             1),
                                                                                                                                                         Axes(
                                                                                                                                                             0)),
                                                                                                                                             ...
                                                                                                                                             tensordot(
                                                                                                                                                 *(Tn[3]
                                                                                                                                                     [1]),
                                                                                                                                                 tensordot(
                                                                                                                                                     tensordot(conj(*(Tn[3][1])), *(op[3][1]), Axes(4), Axes(1)),
                                                                                                                                                     tensordot(
                                                                                                                                                         *(eTb[1]),
                                                                                                                                                         tensordot(
                                                                                                                                                             *(Tn[3]
                                                                                                                                                                 [2]),
                                                                                                                                                             tensordot(tensordot(conj(*(Tn[3][2])), *(op[3][2]), Axes(4), Axes(1)), tensordot(*(eTr[3]), tensordot(*(C[2]), *(eTb[2]), Axes(1), Axes(0)), Axes(1), Axes(0)), Axes(2, 3), Axes(2, 5)), Axes(2, 3, 4), Axes(4, 6, 2)),
                                                                                                                                                         Axes(
                                                                                                                                                             0),
                                                                                                                                                         Axes(
                                                                                                                                                             5)),
...
template <class tensor>
typename tensor::value_type Contract_4x4(
    const std::vector<const tensor *> &C,
    const std::vector<const tensor *> &eTt,
    const std::vector<const tensor *> &eTr,
    const std::vector<const tensor *> &eTb,
...
#define INSTANTIATE_CONTRACT(tensor_type, NROW, NCOL)          \
  template tensor_type::value_type Contract_##NROW##x##NCOL(   \
