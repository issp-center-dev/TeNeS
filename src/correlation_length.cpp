...
template <class ptensor>
ptensor CorrelationLengthCalculator_mf<ptensor>::initial_vector(
...
template <class ptensor>
void CorrelationLengthCalculator_ctm<ptensor>::matvec_horizontal(
    ptensor &outvec, ptensor const &invec, int y) const {
  using mptensor::Axes;
  const auto &lattice = this->lattice;
...
template <class ptensor>
void CorrelationLengthCalculator_ctm<ptensor>::matvec_vertical(
    ptensor &outvec, ptensor const &invec, int x) const {
  using mptensor::Axes;
  const auto &lattice = this->lattice;
  const size_t CHI = C1[0].shape()[0];
  const auto x_orig = x;
...
template <class ptensor>
void CorrelationLengthCalculator_mf<ptensor>::matvec_horizontal(
    ptensor &outvec, ptensor const &invec, int y) const {
  using mptensor::Axes;
  const auto &lattice = this->lattice;
...
template <class ptensor>
void CorrelationLengthCalculator_mf<ptensor>::matvec_vertical(
    ptensor &outvec, ptensor const &invec, int x) const {
  using mptensor::Axes;
  const auto &lattice = this->lattice;
  const auto x_orig = x;
...
template <class ptensor>
ptensor CorrelationLengthCalculator_ctm<ptensor>::matrix_horizontal(
    int y) const {
  using mptensor::Axes;
  const auto &lattice = this->lattice;
  const size_t CHI = C1[0].shape()[0];
...
template <class ptensor>
ptensor CorrelationLengthCalculator_ctm<ptensor>::matrix_vertical(int x) const {
  using mptensor::Axes;
...
template <class ptensor>
ptensor CorrelationLengthCalculator_mf<ptensor>::matrix_horizontal(
    int y) const {
  using mptensor::Axes;
  const auto &lattice = this->lattice;
  int site = lattice.index(0, y);
...
template <class ptensor>
ptensor CorrelationLengthCalculator_mf<ptensor>::matrix_vertical(int x) const {
  using mptensor::Axes;
  using mptensor::Shape;
  const auto &lattice = this->lattice;
...
template class CorrelationLengthCalculator<complex_tensor>;

template class CorrelationLengthCalculator_ctm<real_tensor>;
template class CorrelationLengthCalculator_ctm<complex_tensor>;

template class CorrelationLengthCalculator_mf<real_tensor>;
template class CorrelationLengthCalculator_mf<complex_tensor>;
