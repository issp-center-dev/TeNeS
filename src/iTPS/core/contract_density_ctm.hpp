...
#ifndef TENES_SRC_ITPS_CORE_CONTRACT_DENSITY_CTM_HPP_
...
#define DECLARE_CONTRACT(NROW, NCOL)                                  \
  template <class tensor>                                             \
  typename tensor::value_type Contract_##NROW##x##NCOL##_density_CTM( \
      const std::vector<const tensor *> &C,                           \
      const std::vector<const tensor *> &eTt,                         \
      const std::vector<const tensor *> &eTr,                         \
      const std::vector<const tensor *> &eTb,                         \
 ...
 *  Contract_one_site_density_CTM(..., op).
 */
template <class tensor>
...
template <class tensor>
void TransferMatrix_MatVec_density_CTM(tensor &inoutvec, const tensor &eT1);
