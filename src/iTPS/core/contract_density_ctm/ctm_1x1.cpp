...
typename tensor::value_type Contract_1x1_density_CTM(
  ...
  // 1_1_den.dat
  ////////////////////////////////////////////////////////////
  // (*(op[0][0])*(*(Tn[0][0])*((*(eTt[0])*(*(C[0])**(eTl[0])))*((*(C[1])**(eTr[0]))*(*(C[2])*(*(C[3])**(eTb[0])))))))
  // cpu_cost= 266256  memory= 14352
  // final_bond_order ()
  ////////////////////////////////////////////////////////////
  return trace(
      *(op[0][0]),
      tensordot(
          *(Tn[0][0]),
          tensordot(tensordot(*(eTt[0]),
                              ...
                              Axes(0), Axes(0)),
                    tensordot(tensordot(*(C[1]), *(eTr[0]), Axes(1), Axes(0)),
                              tensordot(*(C[2]),
                                        tensordot(*(C[3]), *(eTb[0]), Axes(0),
                                                  Axes(1)),
                                        Axes(1), Axes(1)),
                              Axes(1), Axes(0)),
                    Axes(0, 2), Axes(0, 2)),
...
//! @cond
INSTANTIATE_CONTRACT_DENSITY(real_tensor, 1, 1);
INSTANTIATE_CONTRACT_DENSITY(complex_tensor, 1, 1);
//! @endcond
