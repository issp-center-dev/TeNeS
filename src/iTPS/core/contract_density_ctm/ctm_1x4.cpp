...
typename tensor::value_type Contract_1x4_density_CTM(
  ...
  // 1_4_den.dat
  ////////////////////////////////////////////////////////////
  // (*(op[0][0])*(*(Tn[0][0])*((*(eTt[0])*(*(C[0])**(eTl[0])))*((*(C[3])**(eTb[0]))*(*(eTt[1])*((*(Tn[0][1])**(op[0][1]))*(*(eTb[1])*(*(eTt[2])*((*(Tn[0][2])**(op[0][2]))*(*(eTb[2])*(*(eTt[3])*((*(Tn[0][3])**(op[0][3]))*(*(eTb[3])*(*(C[1])*(*(C[2])**(eTr[0]))))))))))))))))
...
//! @cond
INSTANTIATE_CONTRACT_DENSITY(real_tensor, 1, 4);
INSTANTIATE_CONTRACT_DENSITY(complex_tensor, 1, 4);
//! @endcond
