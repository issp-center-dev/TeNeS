...
)
{
  ...
  // 2_1_den.dat
  ////////////////////////////////////////////////////////////
  // (*(op[0][0])*(*(Tn[0][0])*((*(eTt[0])*(*(C[0])**(eTl[0])))*((*(C[1])**(eTr[0]))*(*(eTr[1])*((*(Tn[1][0])**(op[1][0]))*(*(eTl[1])*(*(C[2])*(*(C[3])**(eTb[0]))))))))))
        ...
        tensordot(
          *(eTt[0]), tensordot(
          ...
          tensordot(
            ...
            *(eTr[1]), tensordot(
              tensordot(
                *(Tn[1][0]), *(op[1][0]), Axes(4, 5), Axes(0, 1)
              ), tensordot(
                *(eTl[1]), tensordot(
                  *(C[2]), tensordot(
...
//! @cond
INSTANTIATE_CONTRACT_DENSITY(real_tensor, 2, 1);
INSTANTIATE_CONTRACT_DENSITY(complex_tensor, 2, 1);
//! @endcond
