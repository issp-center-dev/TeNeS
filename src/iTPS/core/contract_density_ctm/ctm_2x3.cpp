...
typename tensor::value_type Contract_2x3_density_CTM(
  ...
  // 2_3_den.dat
  ////////////////////////////////////////////////////////////
  // (*(op[0][0])*(*(Tn[0][0])*((*(eTt[0])*(*(C[0])**(eTl[0])))*(((*(Tn[1][0])**(op[1][0]))*(*(eTb[0])*(*(C[3])**(eTl[1]))))*(*(eTt[1])*((*(Tn[0][1])**(op[0][1]))*((*(Tn[1][1])**(op[1][1]))*(*(eTb[1])*(((*(Tn[0][2])**(op[0][2]))*(*(eTt[2])*(*(C[1])**(eTr[0]))))*((*(Tn[1][2])**(op[1][2]))*(*(eTr[1])*(*(C[2])**(eTb[2])))))))))))))
  // cpu_cost= 2.18728e+06  memory= 52784
  // final_bond_order ()
  ////////////////////////////////////////////////////////////
  return trace(
      *(op[0][0]),
              ...
              tensordot(
                  tensordot(
                      tensordot(*(Tn[1][0]), *(op[1][0]), Axes(4, 5),
                                Axes(0, 1)),
                      tensordot(*(eTb[0]),
                                tensordot(*(C[3]), *(eTl[1]), Axes(1), Axes(0)),
                                Axes(1), Axes(0)),
                      Axes(0, 3), Axes(3, 1)),
                  tensordot(
                      *(eTt[1]),
...
//! @cond
INSTANTIATE_CONTRACT_DENSITY(real_tensor, 2, 3);
INSTANTIATE_CONTRACT_DENSITY(complex_tensor, 2, 3);
//! @endcond
