...
typename tensor::value_type Contract_3x1_density_CTM(
  ...
  // 3_1_den.dat
  ////////////////////////////////////////////////////////////
  // (*(op[0][0])*(*(Tn[0][0])*((*(eTt[0])*(*(C[0])**(eTl[0])))*((*(C[1])**(eTr[0]))*(*(eTr[1])*((*(Tn[1][0])**(op[1][0]))*(*(eTl[1])*(*(eTr[2])*((*(Tn[2][0])**(op[2][0]))*(*(eTl[2])*(*(C[2])*(*(C[3])**(eTb[0])))))))))))))
  // cpu_cost= 667664  memory= 26928
  // final_bond_order ()
  ////////////////////////////////////////////////////////////
  return trace(
      *(op[0][0]),
...
//! @cond
INSTANTIATE_CONTRACT_DENSITY(real_tensor, 3, 1);
INSTANTIATE_CONTRACT_DENSITY(complex_tensor, 3, 1);
//! @endcond
