...
typename tensor::value_type Contract_3x2_density_CTM(
  ...
  // 3_2_den.dat
  ////////////////////////////////////////////////////////////
  // (*(op[0][0])*(*(Tn[0][0])*((*(eTt[0])*(*(C[0])**(eTl[0])))*(((*(Tn[0][1])**(op[0][1]))*(*(eTt[1])*(*(C[1])**(eTr[0]))))*(*(eTr[1])*((*(Tn[1][1])**(op[1][1]))*((*(Tn[1][0])**(op[1][0]))*(*(eTl[1])*(((*(Tn[2][0])**(op[2][0]))*(*(eTb[0])*(*(C[3])**(eTl[2]))))*((*(Tn[2][1])**(op[2][1]))*(*(eTr[2])*(*(C[2])**(eTb[1])))))))))))))
...
//! @cond
INSTANTIATE_CONTRACT_DENSITY(real_tensor, 3, 2);
INSTANTIATE_CONTRACT_DENSITY(complex_tensor, 3, 2);
//! @endcond
