...
typename tensor::value_type Contract_4x1_density_CTM(
  ...
  // 4_1_den.dat
  ////////////////////////////////////////////////////////////
  // (*(op[0][0])*(*(Tn[0][0])*((*(eTt[0])*(*(C[0])**(eTl[0])))*((*(C[1])**(eTr[0]))*(*(eTr[1])*((*(Tn[1][0])**(op[1][0]))*(*(eTl[1])*(*(eTr[2])*((*(Tn[2][0])**(op[2][0]))*(*(eTl[2])*(*(eTr[3])*((*(Tn[3][0])**(op[3][0]))*(*(eTl[3])*(*(C[2])*(*(C[3])**(eTb[0]))))))))))))))))
  // cpu_cost= 868368  memory= 33088
  // final_bond_order ()
  ////////////////////////////////////////////////////////////
  return trace(
      *(op[0][0]),
...
//! @cond
INSTANTIATE_CONTRACT_DENSITY(real_tensor, 4, 1);
INSTANTIATE_CONTRACT_DENSITY(complex_tensor, 4, 1);
//! @endcond
