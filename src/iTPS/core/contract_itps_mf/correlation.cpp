...
void StartCorrelation_iTPS_MF(tensor &A, const tensor &Tn, const tensor &op,
                              size_t direction) {
  Axes axes;
...
/* Lambda tensors should be absorbed before entering */
template <class tensor>
void Transfer_iTPS_MF(tensor &A, const tensor &Tn, size_t direction) {
...
/* Lambda tensors should be absorbed before entering */
template <class tensor>
typename tensor::value_type FinishCorrelation_iTPS_MF(const tensor &A,
                                                      const tensor &Tn,
                                                      const tensor &op,
                                                      size_t direction) {
  direction += 2;
  ...
  ////////////////////////////////////////////////////////////
  // (A*(Tn*(conj(Tn)*op)))
  // cpu_cost= 2.0001e+06  memory= 300200
  // final_bond_order ()
  ////////////////////////////////////////////////////////////
  return trace(A,
               tensordot(Tn, tensordot(conj(Tn), op, Axes(4), Axes(1)),
                         axes, axes),
               Axes(0, 1), Axes(0, 1));
}

template void StartCorrelation_iTPS_MF(real_tensor &A, const real_tensor &Tn,
                                       const real_tensor &op, size_t direction);
template void StartCorrelation_iTPS_MF(complex_tensor &A,
                                       const complex_tensor &Tn,
                                       const complex_tensor &op,
                                       size_t direction);

template void Transfer_iTPS_MF(real_tensor &A, const real_tensor &Tn,
                               size_t direction);
template void Transfer_iTPS_MF(complex_tensor &A, const complex_tensor &Tn,
                               size_t direction);

template typename real_tensor::value_type FinishCorrelation_iTPS_MF(
    const real_tensor &A, const real_tensor &Tn, const real_tensor &op,
    size_t direction);
template typename complex_tensor::value_type FinishCorrelation_iTPS_MF(
