  ...
  for (int int_tau = 0; int_tau < nsteps; ++int_tau) {
    for (auto up : simple_updates) {
      if (up.is_onesite()) {
        ...
        Tn[source] = reshape(Tn1_new,mptensor::Shape(Tn[source].shape()[0],Tn[source].shape()[1],Tn[source].shape()[2],Tn[source].shape()[3],up.op.shape()[2],up.op.shape()[0]));
        Tn[target] = reshape(Tn2_new,mptensor::Shape(Tn[target].shape()[0],Tn[target].shape()[1],Tn[target].shape()[2],Tn[target].shape()[3],up.op.shape()[3],up.op.shape()[1]));
      }
    }
      ...
      for (iter_gauge = 0; iter_gauge < maxiter_gauge; ++iter_gauge) {
        for (int parity : {1, -1}) {
		  ...
		  lambda_tensor[source],
              ...
              lambda_tensor[source][source_leg] = lambda_c;
              lambda_tensor[target][target_leg] = lambda_c;
              Tn[source] = reshape(Tn1_new,mptensor::Shape(Tn[source].shape()[0],Tn[source].shape()[1],Tn[source].shape()[2],Tn[source].shape()[3],Tn[source].shape()[4],Tn[source].shape()[5]));
              Tn[target] = reshape(Tn2_new,mptensor::Shape(Tn[target].shape()[0],Tn[target].shape()[1],Tn[target].shape()[2],Tn[target].shape()[3],Tn[target].shape()[4],Tn[target].shape()[5]));
            }
          }
        }
  ...
  for (int int_tau = 0; int_tau < nsteps; ++int_tau) {
	...
	for (int j1 = 0; j1 < Tn[source].shape()[5]; ++j1) {
	  for (int k1 = 0; k1 < Tn[target].shape()[5]; ++k1) {
        ...
        lambda_tensor[source][source_leg] = lambda_c;
        lambda_tensor[target][target_leg] = lambda_c;
        Tn[source] = reshape(Tn1_new,mptensor::Shape(Tn[source].shape()[0],Tn[source].shape()[1],Tn[source].shape()[2],Tn[source].shape()[3],up.op.shape()[2],Tn[source].shape()[5]));
        Tn[target] = reshape(Tn2_new,mptensor::Shape(Tn[target].shape()[0],Tn[target].shape()[1],Tn[target].shape()[2],Tn[target].shape()[3],up.op.shape()[3],Tn[target].shape()[5]));
      }
    }
